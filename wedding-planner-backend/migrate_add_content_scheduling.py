"""
Database migration script to add scheduling fields to content table
...
def migrate():
    ...
    with app.app_context():
        print("=" * 60)
        print("Database Migration: Adding Content Scheduling Fields")
        print("=" * 60)
        print()
                ...
                if col_name not in columns:
                    ...
                    try:
                        with db.engine.connect() as conn:
                            conn.execute(text(f"ALTER TABLE content ADD COLUMN {col_name} {col_type}"))
                            conn.commit()
